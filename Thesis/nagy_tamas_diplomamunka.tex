...
%TODO Write about function shipping and data locality as opposite of data shipping.
...
%TODO Consider the replacement of the figure
%\begin{figure}[h!]
%	\centering
...
\section{Input adatok particionálása és feltöltése} \label{sec:partitioning} %TODO Not the best section title
...
a rendszer mûködtetésének a mûveletek futtatásán túli költsége (adatok feltérképezése és
beolvasása Grain-ek létrehozása) is mérhetõ volt, így kevésbé számításigényes 
...
\textit{Landsat}\footnote{http://landsat.usgs.gov/} mûholdak reflektanciájának kiszámítási módja az alábbi:
...
\textit{légkörön kívüli irradiancia (vagyis az adott hullámsávban beesõ napsugárzás intenzitása)} átlaga, 
...
\begin{figure}[H]
	...
	\caption{A (\subref{fig:gabor}) Gabor függvény  elõállítása az (\subref{fig:gaussian}) Gauss függvénybõl és a (\subref{fig:sinusoid}) szinusz hullámból} %TODO CITE!!!!!!
...
felvételt egy darab szegmensnek tekintjük, melyet minden iterációs lépésben a meghatározott 
\textit{vágási kritérium}nak megfelelõen több részre bontunk. 

\subsubsection{Negyedelõ fa alapú szegmentálás} %TODO Cite
...
\begin{itemize}
	...
	\item Ha egy szegmens egy másik szegmens részhalmaza, akkor két lehetõség van. 
	Ha a szegmens az átfedés határán helyezkedik el, akkor kitöröljük. %TODO What does it mean?
...
Az Orleans rendszer másfajta megközelítést alkalmaz az elosztott feldolgozásra 
amellyel - elméletben - 
kiküszöbölhetõek a fent említett nehézségek. Az egyes ötletek és megvalósításuk a
következõ szakaszban kerülnek ismertetésre.
...
A vágást alkalmazó szegmensek kezdetben az input kép egészét egy darab szegmensnek tekintik, és egy
...
lényegében a teljes input kép bármelyik pixelét és szegmensét elérhetik. Ezzel lényegében az algoritmusok
bármilyen módosítás nélkül elosztottan, tetszõleges számú partíción futtathatók.
...
%TODO If you write this down, you have to implement it
%Az Orleans rendszer ismertetésekor már említésre került, hogy alapértelmezés szerint
%a Grain-ekhez intézett hívások nem "fedhetnek át", tehát egy Grain egyszerre egy hívást
%szolgálhat ki. Mivel ez a jelenlegi helyzetben nem volt kielégítõ (a feldolgozás közben
%egy másik grain-nek szüksége lehet egy megadott pixelértékre, vagy egy szegmensre),
%a feldolgozó Grain-eket elláttuk a Reentrant attribútummal, mely engedélyezi a bejövõ
%hívások kiszolgálásának átfedését. Ez a pixelértékek lekérdezésénél nem okoz problémát
...
területén nyújtott teljesítményétõl. Ha ezen a területen megfelelõ teljesítményt nyújt a rendszer, az
nagy elõrelépést jelentene a távérzékelt felvételek elosztott feldolgozása területén.
...
hogy a tõle kisebb, illetve nagyobb intenzitásértékek között a legnagyobb legyen a különbség.
...
A kiegyensúlyozott hisztogram alapú küszöbölés\cite{balancedHistogramThresholding} két különbözõ
...
egy euklideszi spektrális távolságon alapuló klaszterezõ algoritmus, mely több 
...
intenzitásértékébõl elõállított intenzitástéren operálnak. Ez az intenzitástér az input kép
...
\subsection{Globális mûveletek Orleans alapú feldolgozása}
...
%TODO Do we need to write more here?
