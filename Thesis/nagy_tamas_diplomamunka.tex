...
\begin{abstract}
	...
	azonban más ágens alapú platformok esetén nyújthat megfelelõ teljesítményt, így
...
A modell gyakorlati alkalmazhatóságának a és hatékonyságának vizsgálata az Orleans ágens
...
és komponenseit az \ref{ch:application}. fejezet tartalmazza. A prototípus alkalmazás
...
fedhetnek egymással. Az ábrán a \textit{növényzet}hez (\textit{vegetation}) és a \textit{talaj}hoz (\textit{soil}) tartozó
...
\begin{figure}[h!]
	...
	\includegraphics[width=110mm]{figures/coordinateSystem.png}
...
hogy az eredetileg neki szánt területnél valamennyivel nagyobb területet tartalmazzon (\ref{fig:imagePartitioning}. ábra). Ekkor az 
...
\subsection{A Hadoop keretrendszer} \label{subs:aegisHadoop}
...
hisztogram értékekre. Ennek egy lehetséges feldolgozási módja három különbözõ lépésbõl áll \cite{aegisHadoop}, folyamata a \ref{fig:histogram_map_reduce}. ábrán látható. Elõször egy \textit{Map}
...
által használt osztályokat és mûveleteket tartalmazza. Az alkalmazás komponensdiagramja az \ref{fig:orleansSpectralComponents}. ábrán látható.
...
Ezután elküldi a kiegészített leíró fájlt ezeknek az aktoroknak. A feltérképezés folyamata az \ref{fig:discoverParts}. ábrán látható.
...
A kidolgozott számítási modell, és az Orleans keretrendszer gyakorlati használhatóságát akkor tudjuk igazán megismerni, ha a napjainkban 
...
mûholdak által szolgáltatott adatokra. Ezek az adatok \textit{GeoTIFF}
...
a tesztelés során 2, 4, illetve 8 partíció felhasználásával történtek a mérések.
...
\section{Lokális mûveletek eredményei}  %TODO Consider writing Lokáis mûveletek eredményei	
...
\begin{figure}[h!]
	...
	\caption{Gabor szûrõ alkalmazása közepes méretû felvételekre Orleans platformon}\label{fig:gaborFilter}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{Otsu küszöbölés alkalmazása nagyméretû felvételekre Orleans platformon}\label{fig:otsuThresholding}
\end{figure}

Az Otsu küszöbölés eredményén látható, hogy lokális esetben a mûvelet a kezdeti
...
csak részben - lokális mûveletek esetén - nyújtott megfelelõ teljesítményt. 
...
eredményekbõl látható volt, hogy az egyes szegmensek konzisztenciájának megtartásával járó
