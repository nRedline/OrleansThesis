...
\begin{figure}[H] %TODO Change colors!
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
...
\chapter{Megvalósítás} %TODO Split into two chapters?
...
\begin{figure}[H] %TODO Placement, size
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=135mm]{figures/grainsClassDiagram.png}
...
feldolgozásának mindkét fázisából (tehát az elõfeldolgozás és a tematikus osztályozás részébõl is), 
mindegyik munkaterület-csoportból kerültek kiválasztásra
mûveletek, ezáltal átfogó képet kaphatunk, a képfeldolgozás mely részein alkalmazható hatékonyan a rendszer. 

Mivel az egyes munkaterületekhez tartozó mûveletek eltérõ megközelítést igényelnek, ezáltal
a bemutatás ezeken végighaladva történik.
...
Az \textit{invertálás} talán a legegyszerûbb spektrális mûvelet, melynek során az egyes 
intenzitásértékeket "megfordítjuk" a felvétel minden pixelére. Ezt úgy érhetjük el, hogy a
radiometriai felbontás maximálisan felvehetõ értékébõl kivonjuk az intenzitásértékeket.

\subsubsection{ToA-Reflektancia}
A \ref{ssn:preProcessing}. szakaszban ismertetett \textit{ToA-Reflektancia} a radiometriai korrekciók közé tartozó lokális
...
A \textit{konstans küszöbölés} a küszöbölõ algoritmusok legegyszerûbb formája, ebben az esetben a küszöbértéket egy elõre
meghatározott konstans paraméter formájában adhatjuk meg a mûveletnek. A mûvelet tetszõleges mértékben párhuzamosítható
mindaddig, amíg az összes feldolgozó egység ugyanazzal a küszöbértékkel dolgozik.
...
\textit{átfedések}kel hozzuk létre - ekkor a mûvelet indításakor biztosítanunk kell, hogy a 6
...
\subsubsection{Dobozoló szûrõ}
A \textit{dobozoló szûrõ (box filter)} egy egyszerû lineáris szûrõ, amelynek eredményeként minden pixel értéke az 
õt meghatározott sugárban körülvevõ input pixelek értékének átlaga. A box filter hatása a kép simítása, és
egy bizonyos számú iterációban egymás után alkalmazva közelíti a \textit{Gauss szûrõ}t.

\subsubsection{Gauss szûrõ}
A \textit{Gauss szûrõ (Gaussian blur)} egy olyan szûrési mód, ahol a képet egy \textit{Gauss függvény} segítségével
mossuk el. Egy széles körben használt szûrõ a grafikus képfeldolgozás területén, legfõképp a kép zajosságának 
csökkentésére, a részletesség redukálására valamint éldetektálásra használatos.

\subsubsection{Gabor szûrõ}
A \textit{Gabor szûrõ (Gabor filter)} egy éldetektálásra használatos lineáris szûrõ. 

In the spatial domain, a 2D Gabor filter is a Gaussian kernel function modulated by a sinusoidal plane wave.
 To be continued...
%TODO Finish, cite?

\subsubsection{Fokális mûveletek feldolgozása az Orleans rendszerben}
A fokális mûveletek elosztott feldolgozás során legtöbbször meghatározott mennyiségû plusz adatot igényelnek
a szomszédos partícióktól. Ez a legtöbbször megoldható a korábban ismertetett átfedések definiálásával, mivel
...
adatok feldolgozásához, azonban egyrészt késõbbi mûvelettípusokra való felkészülés, 
másrészt tesztelési szempontok miatt, implementálásra került a \textit{Grain}-ek egymás közti direkt kommunikációján
alapuló feldolgozás. 
%TODO Consider recompose
%TODO Add Filtering class diagram?
...
A virtuális raszter bevezetésével és használatával megoldást adunk arra a problémára ha egy fokális mûvelet végrehajtásához
csak egy olyan input partíciók állnak rendelkezésre, melyek nem rendelkeznek átfedésekkel. A következõ fejezetben
...
\subsubsection{Regionális mûveletek MapReduce alapú feldolgozása}
\subsubsection{Regionális mûveletek feldolgozása az Orleans rendszerben}
