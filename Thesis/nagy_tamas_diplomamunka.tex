...
\begin{abstract}
	...
	Ezzel párhuzamosan a számítástudomány fókusza az elosztott rendszerek felé irányult.
	...
	Jelen dolgozat célja egy olyan számítási modell kifejlesztése, mely lehetõvé teszi
	a távérzékelt felvételek kiértékelését ágens alapú platformokon. 
	A feldolgozásban részt vevõ mûveletek mûvelettípusai elosztott feldolgozás során
	eltérõ megközelítést igényelnek. A dolgozat bemutatja a mûvelettípusok ágens
	alapú feldolgozására kifejlesztett megoldásokat, a számítási modell teljesítményét pedig
	az Orleans keretrendszer által biztosított környezetben méri. Az elért eredmények 
	ezután összevetésre kerülnek az egyik  legnépszerûbb MapReduce alapú keretrendszer, 
	a Hadoop platform teljesítményével is.
	
	A mért eredmények azt mutatják, hogy az Orleans által nyújtott környezet egyes 
	mûvelettípusoknál jobb-, másoknál gyengébb teljesítményt ért el, mint a Hadoop rendszer,
	így az összes mûvelettípus elvégzésére nem alkalmas, gyakorlati alkalmazhatósága nagyban
	függ az elvégzendõ feladattól. A dolgozat során kifejlesztett számítási modell 
	azonban más ágens alapú platformok esetén megfelelõ teljesítményt nyújthat, így
	több késõbbi kutatás alapjául szolgálhat.
...
történelmében most a legmagasabb, és folyamatosan emelkedik. A rendelkezésre álló megújuló természeti erõforrások azonban végesek és sebezhetõek.
A növénytermesztés területén például a fejlõdõ növényzetet több veszélyforrás 
is fenyegetheti (aszály, belvíz, gyomnövények, állati kártevõk).
...
Látható, hogy természeti erõforrásaink folyamatos, helyszíni megfigyeléssel történõ felmérése
hatalmas ráfordított emberi energiával és pénzbeli költségekkel jár. 
...
a feldolgozás teljesítményének növelésére irányult. A mûholdak által szolgáltatott felvételek részletességüktõl
...
Az elosztott rendszerek fejlõdése során több különbözõ fejlesztési \textit{paradigma} jelent meg, 
napjainkban pedig ezek közül kettõt alkalmaznak széleskörûen: a nagy
...
Jelen dolgozat célja egy olyan \textit{számítási modell} kifejlesztése és tesztelése, melynek 
segítségével az aktor modellre épülõ rendszerek is felhasználhatóak a távérzékelt felvételek 
feldolgozásában. A modell biztosítja, hogy a feldolgozásban részt vevõ mûveletek
módosítás nélkül végrehajthatóak legyenek az aktor modell által nyújtott elosztott
környezetben, annak minden elõnyét (\textit{skálázhatóság, bõvíthetõség, stb.}) kihasználva. 
...
melyek az ágens alapú környezetben egyedi megközelítést igényeltek. 
Az alkalmazott ötletek az egyes mûvelettípusokkal együtt kerülnek bemutatásra. 

A modell gyakorlati alkalmazhatóságának a és hatékonyságának vizsgálata az \textit{Orleans} ágens
alapú platformon történt, melynek teljesítménye összevetésre kerül 
az egy számítógépen történõ, \textit{lokális} feldolgozás teljesítményével, valamint a MapReduce rendszerek 
zászlóshajójának számító \textit{Hadoop} platformon mért eredményekkel.
\newline
...
Az Orleans platform teljesítményének
...
bemutatása után az egyes vizsgált mûveletek ismertetése következik, ezt az \ref{ch:algorithms}.
...
multispektrális szenzorok sávszáma 5 és 10 között van, és a reflektív tartomány mellett 
...
radiometriai felbontással rendelkezik. Napjaink legelterjedtebb nagyfelbontású multispektrális
szenzorainál, a \textit{Landsat}\footnote{http://landsat.usgs.gov/}, illetve a 
\textit{SPOT}\footnote{https://spot.cnes.fr/} (\textit{Satellite Pour l'Observation de la Terre}) mûholdaknál
a \textit{8-12 bites} radiometriai felbontás jellemzõ, mely \textit{256}, illetve \textit{4096} 
különbözõ intenzitásszintnek felel meg.
...
\begin{itemize}
	...
	\item \textbf{Természeti erõforrások megfigyelése }
	\newline
	Földhasználat, vadvilági területek megfigyelése. A távérzékelt adatok felhasználásával minimalizálhatjuk a
...
A felvételkiértékelés célja a felszínborítás, valamint a földfelszíni objektumok fizikai jellemzõinek megállapítása. A felvételkiértékelés két alapvetõ módszere a 
\textit{vizuális interpretáció} és a \textit{kvantitatív, numerikus kiértékelés}. 

Vizuális interpretáció során a tematikus döntéseket a felvételen \textit{látható} információ alapján hozza meg a
...
Pontonkénti mûveletek például az egyes \textit{küszöbölõ eljárás}ok, melyek során a spektrális intenzitás intervallumot
...
jellemzõen nem pontokba tömörülnek, hanem rendelkeznek bizonyos mértékû szórással, akár át is
fedhetnek egymással. 

\begin{figure}[h!]
	\centering
	\captionsetup{width=1\textwidth, justification=centering}
	\includegraphics[width=100mm]{figures/coordinateSystem.png}
	\caption{Különbözõ felszíndarabok sugárzása  $x_1=0.5\si{\micro\metre}$ 
		és $x_2=0.9\si{\micro\metre}$ hullámhosszon \label{fig:intensitySpaces}}
\end{figure}

Az ábrán a \textit{növényzet}hez (\textit{vegetation}) és a \textit{talaj}hoz (\textit{soil}) tartozó
intenzitások például több pontban is összemosódnak. A klaszterezés során
...
\begin{itemize}
	...
	lényegében egy pontból pontba történõ transzformáció (\ref{fig:localOperation}. ábra). Ebbe a csoportba tartoznak az egyszerûbb képi
	...
	értékek felhasználásával dolgozik (\ref{fig:focalOperation}. ábra). Ide tartoznak például a különbözõ \textit{szûrõ} mûveletek.
\end{itemize}
...
\begin{itemize}
	...
	mely a feldolgozás során dinamikusan változhat (\ref{fig:zonalOperation}. ábra). Regionális mûvelet például a legtöbb \textit{szegmentáló}
	algoritmus.
	
	\item \textit{Globális mûvelet:} Az eredménykép minden képpontjának elõállításához rendelkeznie kell a teljes
	forráskép ismeretével (\ref{fig:globalOperation}. ábra). Ide tartoznak például azok a \textit{küszöbölõ eljárások}, melyek
...
\section{A feldolgozás teljesítménye} %TODO Rename
...
A teljesítménynövelés egy másik lehetséges módja az évek óta kutatott \cite{distributedImageProcessing}, és a számítási
felhõk elterjedésével az utóbbi idõben teret nyert \textit{elosztott feldolgozás}.

% ------------------------------------------------------------------------------

\chapter{Elosztott fejlesztési platformok} \label{ch:distributedSystems}
...
egymással hálózaton keresztül kommunikálnak. A komponensek saját processzorral és memóriával 
rendelkeznek (Szemben a párhuzamos rendszerekkel, amelyek a memóriát megosztják egymással) \cite{distributedParallel}.
...
köztes elemet, melynek kulcsa megegyezik (\ref{fig:mapReduceProcess}. ábra).
...
\begin{figure}[h!]
	...
	\includegraphics[width=150mm]{figures/mapReduce.PNG}
	\caption{A \textit{MapReduce} folyamat \label{fig:mapReduceProcess}}
\end{figure}

\section{A Hadoop keretrendszer} \label{subs:aegisHadoop}
...
\subsection{Térinformatikai alkalmazása} \label{subs:hadoopRemoteSensingApplication}
...
\subsubsection{Adatkezelés}
Távérzékelt felvételek elosztott feldolgozása során a feldolgozandó kép alkotóelemeit valamilyen tulajdonság alapján 
...
hogy az eredetileg neki szánt területnél valamennyivel nagyobb területet tartalmazzon (\ref{fig:imagePartitioning}. ábra).
...
egymással kommunikálva biztosítják a számításokhoz szükséges adatokat, a Hadoop esetén azonban 
erre nincs lehetõség.

A legtöbb globális mûvelet is elvégezhetõ particionált módon, itt azonban mindenképp szükséges 
egy központi összefuttató lépés - ennek módszere az alkalmazott mûvelettõl függ.

\subsubsection{Mûveletek futtatása}
...
A \textit{hisztogram kiegyenlítés} mûvelete például globális mûvelet, melynek szüksége van az egész képre vonatkozó
hisztogram értékekre. Ennek egy lehetséges MapReduce alapú feldolgozási módja három különbözõ lépésbõl áll \cite{aegisHadoop}, 
folyamata a \ref{fig:histogram_map_reduce}. ábrán látható. Elõször egy \textit{Map}
...
\begin{figure}[H] %TODO Reposition
	\centering
	\includegraphics[width=150mm]{figures/histogram_map_reduce.png}
...
\textit{módosíthatatlan (immutable)} üzeneteken keresztül kommunikálnak egymással (\ref{fig:actorModel}. ábra).
Minden aktor rendelkezik egy "\textit{postaládával}", ahová az üzenetek érkeznek, és egy 
\textit{izolált állapot}tal, melybõl az aktor szükség esetén újra létrehozható.
...
\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/actorModel.png}
...
kiküszöbölésükre való törekvés hívta életre a \textit{Microsoft Orleans} keretrendszert \cite{orleansMSR}.

\section{A Microsoft Orleans platform}
Az \textit{Orleans} egy - a \textit{Microsoft Research} által fejlesztett - keretrendszer, 
mely lehetõséget ad megbízható és skálázható felhõalapú alkalmazások fejlesztésére \cite{dotnetOrleans, orleansMSR}.
...
egy alkalmazás fordulói elindításuk után megszakítás nélkül végrehajtódnak, az ütemezõ nem
szakítja meg a futtatást, és nem veszi el az erõforrásokat egy végrehajtás alatt álló fordulótól.
Az Orleans ütemezõje az aktorok futtatásához és ütemezéséhez tipikusan annyi számítási szálat
használ, amennyi processzormaggal rendelkezik az adott rendszer. 
...
Az Orleans rendszer gyakorlati alkalmazhatóságának vizsgálatához
szükséges volt egy olyan prototípus alkalmazás megvalósítása, 
melynek segítségével tesztelhetõek a keretrendszer képességei távérzékelt felvételek feldolgozásában. 
A fejezet célja ezen alkalmazás felépítésének és komponenseinek bemutatása.

Az Orleans térinformatikai kiegészítéséhez szükséges volt egy olyan keretrendszer integrációja és felhasználása,
amelynek segítségével egységesen kezelhetünk és dolgozhatunk fel téradatokat. 
Így esett a választás az \textit{AEGIS} keretrendszerre.
...
álló térinformatikai keretrendszer. A projekt fõ célja az oktatás mellett térinformatikai algoritmusok, megoldások \textit{kutatás}a és \textit{fejlesztés}e.
...
mûveletekkel, amelyek a dolgozatban vizsgálatra kerültek. Ezek alapján a rendszer megfelelõ környezetet
...
\subsubsection{Adat-, illetve mûveleti modell}
A rendszer adatmodellje (\ref{fig:aegis_data_model}. ábra) az \textit{Open Geospatial Consortium (OGC)} szervezet által definiált
\textit{Simple Feature Access (SFA)} szabvány \cite{sfa}. A szabványban az összes
...
A mûveletet az Operation osztály \textit{Execute()} metódusával futtathatjuk, mely
...
Az AEGIS támogatja a \textit{Hadoop} architektúrával történõ adatkezelést, illetve feldolgozást \cite{aegisHadoop}.
...
\section{Programszerkezet} %TODO Rename
...
által használt osztályokat és mûveleteket tartalmazza. Az alkalmazás komponensdiagramja a \ref{fig:orleansSpectralComponents}. ábrán látható.
...
A Hadoop ismertetése során említésre került, hogy távérzékelt felvételek
elosztott feldolgozása esetén az input adatokat valamilyen tulajdonság alapján feldaraboljuk
és az egyes darabokat külön számítási egységekkel dolgozzuk fel. Az Orleans rendszer
esetén is ez történt, azonban a particionálás mûvelete kiegészült egy 
\textit{partíció leíró} fájl generálásával (\ref{fig:partitionDescriptor}. ábra), amely tartalmazza az eredeti kép dimenzióit,
...
A fenti limitációk megkerülése érdekében egy saját stratégia került alkalmazásra (\ref{fig:getOneGrainFromEachSilo}. ábra): bevezetünk egy
...
Ezután elküldi a kiegészített leíró fájlt ezeknek az aktoroknak. A feltérképezés folyamata a \ref{fig:discoverParts}. ábrán látható.
...
\chapter{Távérzékelési algoritmusok} \label{ch:algorithms}
...
A \textit{Landsat} mûholdak által szolgáltatott felvételekre a reflektancia kiszámítása az alábbi módon történik:
...
\subsection{Fokális mûveletek feldolgozása Orleans platformon}
A fokális mûveletek elosztott feldolgozása során az egyes partíciók legtöbbször meghatározott mennyiségû plusz 
adatot igényelnek
szomszédaiktól. Ez legtöbbször megoldható a korábban ismertetett átfedések definiálásával, mivel
...
és egy \textit{szerviz}en keresztül éri el az egyes adatokat (\ref{fig:virtualRaster}. ábra). A szerviz (\textit{OrleansRasterService}) 
...
- mint a neve is utal rá - egy összevonás alapú, iteratív szegmentálási módszer \cite{bestMerge,bestMerge2}. Az algoritmus kezdõállapotában minden pixelértéket 
...
vagy a homogenitási kritérium alapján már nem tud több szegmenst összevonni.
...
meghatározott vágási kritérium alapján több szegmensre osztják fel - például a korábban ismertetett
...
szükséges, hogy a szegmentáló mûvelet által létrehozott szegmensek minden lépésben 
...
Ha az egységeknek biztosítani tudnánk egy olyan szegmensgyûjteményt, amely - elméletben - a feldolgozandó
...
Az \ref{fig:segmentUpdate}. ábrán egy olyan eset látható, amikor egy lokális szegmenst
egy távoli szegmenssel kapcsolunk össze. A Szegmens szerviz (\textit{Segment Service}) ekkor a távoli szegmenst 
...
egység tartalmazni fogja, mivel nem egyértelmûen eldönthetõ, hogy melyik partícióhoz tartozik (\ref{fig:virtualSegmentCollection}. ábra).
...
\begin{figure}[H] 
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=120mm]{figures/virtualSegmentCollection.png}
	\caption{Mindkét Grain virtuális szegmensgyûjteménye által tartalmazott szegmens. \label{fig:virtualSegmentCollection}}
\end{figure}
...
ad biztosan optimális eredményt. Ez kiküszöbölhetõ, ha a hisztogram "kiugró" értékeit nem vesszük 
...
spektrális tulajdonságairól, így ha elég sok iteráció engedélyezett, a kezdõ klaszterközéppontok
elhelyezkedésének nincs jelentõsége.
...
újraparticionálhatjuk õket. Ezután az egyes partíciókat újabb \textit{Map} függvényekkel
...
a feldolgozó Map függvények között nem okoz jelentõs teljesítménycsökkenést. 
...
\subsection{Globális mûveletek feldolgozása Orleans platformon} \label{subs:globalProcessing}
...
kiszámítjuk, és az \ref{sec:partitioning}. szakaszban ismertetett partíció leíró fájlba
...
A kidolgozott számítási modell és az Orleans keretrendszer gyakorlati használhatóságát akkor tudjuk igazán megismerni, ha napjaink 
legtöbbször használt mûholdai által szolgáltatott adatok feldolgozására vizsgáljuk a 
teljesítményét. A tesztelés során így egy nagyméretû (1GB) Landsat-, illetve egy közepes méretû
(250MB) SPOT mûholdfelvételt dolgoztunk fel. Ezek az adatok \textit{GeoTIFF}
...
Az AEGIS keretrendszer segítségével végeztük el továbbá a képek felosztását is.

Ahhoz, hogy minél részletesebb képet kapjunk a rendszer teljesítményérõl, 
a mûveleteket két különbözõ környezetben futtattuk. 
Az elosztott környezetet tíz darab átlagos számítási-, illetve
I/O teljesítménnyel
...
A vizsgálatok azt mutatták, hogy elosztott feldolgozás esetén nyolc partíció
felhasználásával még a legnagyobb méretû felvételek is megfelelõen alacsony méretû darabokat eredményeznek,
több partícióval már nagyobb költsége lenne a particionálásnak, 
illetve az egyes partíciók összefésülésének a mûveletek végrehajtása után. Ennek megfelelõen
a tesztelés során 1, 2, 4, illetve 8 partíció felhasználásával történtek a mérések.

%esett a választás napjaink két meghatározó
%mûholdcsaládjára, egy nagyméretû (\textit{1GB}) \textit{Landsat}-, és egy közepes méretû (\textit{250MB})
%\textit{SPOT}
%mûholdfelvételre. 
...
végrehajtáshoz. 
Egy napjainkban erõsnek mondható számítási-, és I/O kapacitással rendelkezõ \textit{PC} 
segítségével tudtuk a 
\textit{nagy teljesítményû lokális feldolgozás}t tesztelni, illetve a fent említett
klaszter egyik számítógépén is futtattunk teszteket, 
melynek segítségével információt kaptunk a \textit{közepes teljesítményû lokális feldolgozás}
teljesítményérõl. Az eredmények közlése a mûveletek bemutatásához hasonlóan
munkaterület-csoportonként történik.

\section{Lokális mûveletek eredményei}  
Mivel a választott lokális algoritmusok az eredménykép minden
...
invertálás mûvelet (\ref{fig:inversion}. ábra) végrehajtása során
csak négy partíció elosztott feldolgozásával tudta az Orleans
rendszer megközelíteni a nagy teljesítményû lokális feldolgozást, a Hadoop pedig még 
nyolc partíció felhasználásával sem tudta elérni a nagy teljesítményû lokális környezeten
mért teljesítményt. A közepes teljesítményû lokális feldolgozás azonban jóval gyengébb
teljesítményt nyújtott, az Orleans rendszer itt már két partíció felhasználásával is
jelentõsen jobb eredményt produkált.

\begin{figure}[H]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=145mm]{figures/diagrams/inversion.png}
	\caption{Invertálás mûvelet végrehajtása nagyméretû felvételekre}\label{fig:inversion}
\end{figure}

Ha az elosztott rendszerek egymáshoz képesti teljesítményét vizsgáljuk, 
...
négyszer nagyobb lesz, mint az input adat. A mûvelet végrehajtásának eredménye
a \ref{fig:toaref}. ábrán látható.

\begin{figure}[h]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=145mm]{figures/diagrams/toaref.png}
	\caption{A ToA-Reflektancia mûvelet végrehajtása nagyméretû felvételekre}\label{fig:toaref}
\end{figure}

Az eredményekbõl megfigyelhetõ, hogy a feldolgozó rendszer I/O teljesítménye a megnövelt
méretû eredmény miatt jelentõsebb mértékben számít bele a rendszer egészének teljesítményébe.
Így a nagy teljesítményû lokális feldolgozás teljesítményét csak 8 partíció felhasználásával
tudta az Orleans rendszer megközelíteni, a Hadoop teljesítményében pedig még mindig jelentõs
mértékben érzékelhetõ a rendszer inicializálásának költsége.
Fontos megjegyezni, hogy a nagyméretû eredménykép elõállítása jelentõs memóriaigénnyel jár.
Az elosztott klaszterben szereplõ számítógépek nem rendelkeztek elegendõ memóriával ahhoz, 
hogy a mûveletet
egy egész Landsat felvételre végre tudják hajtani, így a közepes teljeítményû lokális végrehajtás, 
illetve az elosztott platformok egy partíció felhasználásával nem tudták végrehajtani a mûveletet,
ezért nem szerepelnek az eredmények között.
...
a nagyteljesítményû lokális feldolgozással, ha elegendõ méretû memória,
illetve megfelelõ teljesítményû háttértár áll rendelkezésre. 
Az elosztott rendszerek teljesítményében egy másik fontos szempont, hogy a
...
\section{Fokális mûveletek eredményei}
...
\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/diagrams/gaussianBlur.png}
...
A mért eredmények azonban nem ezt bizonyították. Az alábbi, \ref{fig:gaborFilter}.
...
\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/diagrams/gaborFilter.png}
	\caption{Gabor szûrõ alkalmazása közepes méretû felvételekre}\label{fig:gaborFilter}
\end{figure}
...
\section {Regionális mûveletek eredményei}
Regionális mûveletek esetén két különbözõ típusú mûveletet vizsgáltunk. Vágás alapú
szegmentáló mûveletek esetén tekinthetjük a particionálás mûveletét egy kezdeti vágó lépésnek. 
Ennek segítségével az egyes partícióknak nincs szükségük plusz információra a szomszédos
partícióktól, lényegében minden szükséges adat lokálisan rendelkezésre áll.
Ennek megfelelõen a Hadoop rendszeren történõ feldolgozás során nincs szükség átfedések
definiálására, az Orleans platformon való végrehajtás esetén pedig nincs szükség a
feldolgozó egységek egymás közötti kommunikációjára. A \ref{fig:quadTreeSegmentation}. ábra
a vágáson alapuló negyedelõ fa alapú szegmentálás végrehajtásának eredményeit mutatja, melyen 
látható, hogy az Orleans és a Hadoop keretrendszer is megfelelõ teljesítményt nyújt elegendõ
mennyiségû partíció felhasználása esetén.

\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/diagrams/quadTreeSegmentation.png}
	\caption{Negyedelõ fa alapú szegmentálás végrehajtása nagyméretû felvételekre}\label{fig:quadTreeSegmentation}
\end{figure}

\subsubsection{Virtuális szegmensgyûjtemény használata}
Az összevonás alapú szegmentáló mûveleteknek szükségük lehet a szomszédos partíciók által
tartalmazott adatokra. Ehhez a Hadoop rendszerben átfedéseket definiálunk az egyes darabokra, így
egy partíció az õt körülvevõ darabokból is tartalmaz részeket. A megoldás elõnye, hogy minden adat
lokálisan rendelkezésre áll a feldolgozó egységek számára, azonban az átfedések méretének
meghatározása nehéz feladat, ráadásul az egyes egységek így a szükségesnél nagyobb területet
dolgoznak fel. Az Orleans keretrendszer ezzel szemben csak a szükséges méretû területtel rendelkezik,
és a szükséges plusz adatot a többi feldolgozó egységgel kommunikálva szerzi meg.
A mûveletek egy virtuális szegmensgyûjteményen operálnak, így minden feldolgozó egység minden adathoz 
hozzáfér. Arra keressük a választ, hogy az Orleans rendszer biztosít-e olyan sebességû kapcsolatot
az egyes Grain-ek között, melynek segítségével a virtuális szegmensgyûjteményen alapuló mûveletek
is megfelelõ sebességgel operálhatnak. Az egyik vizsgált összevonás alapú mûvelet, a szekvenciális
csatoláson alapuló szegmentálás eredményei a \ref{fig:sequentialCoupling}. ábrán láthatóak.

\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/diagrams/sequentialCouplingSegmentation.png}
	\caption{Szekvenciális csatolás alapú szegmentálás végrehajtása közepes méretû felvételekre}\label{fig:sequentialCoupling}
\end{figure}

A szegmentálás eredményén megfigyelhetõ, hogy a virtuális szegmensgyûjteményen alapuló feldolgozás
nem tud megfelelõ teljesítményt nyújtani az Orleans rendszerben. Míg a Hadoop rendszer folyamatosan
közelíti, 4 partíció esetén pedig már meg is haladja a nagy teljesítményû lokális feldolgozás eredményét,
addig az Orleans rendszer a partíciók növelésével egyre gyengébb teljesítményt nyújt. Ennek oka a 
partíciók közti kommunikáció költségességében keresendõ - minél több partíciót veszünk be a feldolgozásba,
a virtuális szegmensgyûjtemény konzisztenciájának fenntartásához annál több kommunikációra van szükség
az egyes Grain-ek között. 

\section{Globális mûveletek eredményei}
...
csak az egyik mûvelet futtatásának eredménye kerül elemzésre (\ref{fig:otsuThresholding}. ábra).

\begin{figure}[h!]
	\centering
	\captionsetup{width=0.8\textwidth, justification=centering}
	\includegraphics[width=150mm]{figures/diagrams/otsuThresholding.png}
	\caption{Otsu küszöbölés alkalmazása nagyméretû felvételekre}\label{fig:otsuThresholding}
\end{figure}

Az Otsu küszöbölés eredményén látható, hogy mindkét lokális esetben a mûvelet a kezdeti
hisztogram-kiszámítás miatt magasabb mûveletigénnyel rendelkezik. 
Így az Orleans
rendszer a közepes teljesítményû lokális eredményhez képest egyértelmûen hatékonyabb, a nagy
teljesítményû lokális eredményt már két partíció használata esetén is megközelíti, 
több darab felhasználásával pedig jelentõsen gyorsabb.
...
A távérzékelt felvételek felhasználási területeinek széleskörû bõvülésével
egyre nagyobb az igény a felvételek által szolgáltatott adatok gyors és olcsó 
kiértékelésére. A feldolgozás sebességét leghatékonyabban elosztott rendszerek
használatával növelhetjük, azonban a feldolgozásban részt vevõ mûveletek
elosztott végrehajtása gyakran eltérõ megközelítéseket igényel.

A feladatok osztott végrehajtása során leggyakrabban olyan keretrendszereket 
használunk fel, amelyek több elosztott rendszerekkel kapcsolatos nehézséget
megoldanak és elrejtenek, így a fejlesztõk a konkrét feladat megoldására 
koncentrálhatnak. Az osztott paradigmák közül a legkedveltebb, MapReduce 
alapú rendszerek mellett az aktor modellre épülõ keretrendszerek is egyre 
nagyobb népszerûségnek örvendenek. 

A dolgozat célja egy aktor alapú számítási modell kifejlesztése és az 
Orleans keretrendszeren történõ tesztelése volt
távérzékelt felvételek feldolgozásának területén.
Ehhez szükséges volt a keretrendszer kiterjesztése egy egységes téradat-, illetve 
mûveleti modellel melyet az AEGIS térinformatikai keretrendszer biztosított.
Az AEGIS komponens alapú architektúrája lehetõvé tette a bõvíthetõséget és a 
könnyû integrációt az Orleans keretrendszerrel, mindemellett tartalmazta 
a diplomamunka által vizsgált mûveletek implementációit is. Ezen mûveletek
kiterjesztése elosztott feldolgozásra mûvelettípusonként eltérõ
megközelítést igényelt. Egy megadott partíció feldolgozásához szükséges lehetett
olyan adatok ismerete, amelyet egy másik partíció tartalmazott. Az Orleans rendszerben 
ezen adatok elérését az egyes aktorok biztosították egymás között. Ahhoz, hogy az
AEGIS keretrendszerben implementált algoritmusokat módosítás nélkül felhasználhassuk,
a keretrendszer adatmodellét bõvítettük ki olyan osztályokkal 
(\textit{Virtuális Raszter, Virtuális Szegmensgyûjtemény}), melyek az aktorok
egymás közötti kommunikációját kezelték és elrejtették az egyes mûveletek elõl.

A vizsgált mûveletek bemutatásával átfogó képet kaphattunk a távérzékelt
felvételek feldolgozásának folyamatáról, egyes állomásairól és a mûveletek típusairól, 
a rendszer teljesítményét pedig részletes mérések végrehajtásával állapítottuk meg.
A mért eredmények emellett összehasonlításra kerültek a felvételek lokális 
környezeten történõ feldolgozásának teljesítményével, illetve a MapReduce paradigma
zászlóshajójának számító Hadoop rendszeren mért teljesítménnyel.

Az eredmények azt mutatták, hogy a kifejlesztett számítási modell az Orleans keretrendszeren
csak részben nyújtott megfelelõ teljesítményt. A keretrendszer fõként azokban az esetekben
bizonyult hatékonynak, ha az összes szükséges adat lokálisan rendelkezésre állt.
Az aktorok egymás közti információcseréjére építve 
már az egyszerûbb, fokális mûveletek feldolgozása során sem volt tapasztalható 
olyan mértékû teljesítmény, amely megközelítené a versenytársak hatékonyságát.
Fokális mûveleteknél egy javítási lehetõség lett volna az egyes aktorok közti információk
gyorsítótárazása, így az aktorok közti szükséges kommunikációk száma csökkenthetõ.
A regionális, szegmentáló mûveletek esetén azonban ez az optimalizáció hatástalan lett volna. Az 
eredményekbõl látható volt, hogy a virtuális szegmensgyûjteményben a szegmensek konzisztenciájának megtartásával járó
folyamatos lekérdezõ-, illetve frissítõ mûveletek a rendszer jelentõs mértékû teljesítményromlásához
vezettek. Ugyanezen okok miatt a globális, klaszterezõ mûveletek
végrehajtása nem volt lehetséges a rendszerben. Az egyes prototípus megvalósítások a
megnövekedett hálózati kommunikáció miatt nem produkáltak mérhetõ eredményt.

Összességében tehát elmondható, hogy az Orleans keretrendszer csupán a lokális mûvelettípusok
feldolgozásánál alkalmazható hatékonyan, fokális, regionális, illetve globális mûveletek 
esetén azonban vagy gyenge, vagy nem mérhetõ eredményeket mutatott.
A dolgozat során kifejlesztett modell, valamint megvalósítása (virtuális raszter, 
virtuális szegmensgyûjtemény) egy késõbbiekben megjelenõ, megfelelõ teljesítményt
nyújtó aktor alapú keretrendszerben azonban ideális kiindulási alapot adhat a kutatáshoz.
...
\bibitem{richards} Richards, J. A.: \textit{Remote Sensing Digital Image Analysis, An Introduction, fifth edition}, Springer, 2013.
...
\bibitem{aegis} Giachetta, R.: \textit{AEGIS - A state-of-the art component based spatio-temporal framework}, OSGeo Journal 13.1, 2014.

\bibitem{aegisHadoop} Giachetta, R., Fekete, I.: \textit{A case study of advancing remote sensing image analysis}, Acta Cybernetica 22, 2015.

\bibitem{shapefile} \textit{ESRI Shapefile technical description}, An ESRI White Paper, 1998.

\bibitem{geotiff} Ritter, N., Ruth, M.: \textit{The GeoTiff data interchange standard for raster geographic images}, International Journal of Remote Sensing 18.7, 1997.

\bibitem{dotnetOrleans} \textit{Microsoft Orleans, A straightforward approach to building distributed, high-scale applications in .NET}, http://dotnet.github.io/orleans, 2016.

\bibitem{orleansMSR} Bernstein, P., Bykov, S., Geller, A., Kliot, G., Thelin, J.: \textit{Orleans: Distributed Virtual Actors for Programmability and Scalability}, MSR Technical Report, 2014.

\bibitem{mapAlgebra} \textit{Map Algebra: Global, Zonal, Focal and Local Operations}, http://gisgeography.com/map-algebra-global-zonal-focal-local, 2016.

\bibitem{distributedImageProcessing} Giloi, W.: \textit{Distributed Image Processing}, Advances in Digital Image Processing, 1979.

\bibitem{distributedParallel} Andrews, Gregory R.: \textit{Foundations of Multithreaded, Parallel, and Distributed Programming}, Addison-Wesley, 2000.

\bibitem{mapReduce} Dean, J., Ghemawat, S.: \textit{MapReduce: simplified data processing on large clusters}, Communications of the ACM 51.1, 2008.

\bibitem{spatialHadoop} Eldawy, A., Mokbel, M. F.: \textit{SpatialHadoop: An Efficient Mapreduce Framework for Spatial Data}, IEEE 31st International Conference, 2015.

\bibitem{hadoopGIS} Aji, A., Wang, F., Vo, H., Lee, R., Liu, Q., Zhang, X., Saltz, J.: 
\textit{Hadoop GIS: a high performance spatial data warehousing system over mapreduce}, Proceedings of the VLDB Endowment, 2013.

\bibitem{actorOrig} Hewitt, C., Bishop, P., Greif, I., Smith, B., Matson, T., Steiger, R.: \textit{Actor Induction and Meta-evaluation},  Proceedings of the 1st annual ACM SIGACT-SIGPLAN symposium on Principles of programming languages, 1974.

\bibitem{actorErlang} Vinoski, S.: \textit{Concurrency with Erlang}, IEEE Internet Computing, 2007.

\bibitem{erlang} Armstrong, J.: \textit{Erlang}, Communications of the ACM 53, 2010.

\bibitem{actorScala} Haller, P., Odersky, M.: \textit{Scala Actors: Unifying thread-based and event-based programming}, Theoretical Computer Science 410.2, 2009.

\bibitem{actorAkka} Gupta, M.: \textit{Akka essentials}, Packt Publishing Ltd, 2012.

\bibitem{sfa} Herring, J.: \textit{OpenGIS Implementation Standard for Geographic information-Simple feature access-Part 1: Common architecture}, OGC Document 4.21, 2011.

\bibitem{gaussianBlur} Nixon, M.: \textit{Feature Extraction and Image Processing}, Academic Press, 2008.

\bibitem{gaborFilter} Gabor, D.: \textit{Theory of communication. Part 1: The analysis of information}, Electrical Engineers-Part III: Radio and Communication Engineering, Journal of the Institution 93.26, 1946.

\bibitem{gaborFilter2} Fogel, I., Sagi, D.: \textit{Gabor filters as texture discriminator}, Biological cybernetics, 1989.

\bibitem{gaborFilterApplications} Weldon, T. P., Higgins, W. E., Dunn, D. F.: \textit{Gabor filter design for multiple texture segmentation}, Optical Engineering 35.10, 1996.

\bibitem{gaborFilterVisualisation} Prasad, V. S. N., Domke, J.: \textit{Gabor filter visualization}, J. Atmos. Sci 13, 2005.

\bibitem{quadTree} Samet, H.: \textit{The quadtree and related hierarchical data structures}, ACM Computing Surveys 16.2, 1984.

\bibitem{sequentialCoupling} Kettig, R. L., Landgrebe, D. A.: \textit{Classification of multispectral image data by extraction and classification of homogeneous objects}, Geoscience Electronics, IEEE Transactions 14.1, 1976.

\bibitem{sequentialCoupling2} László, I., Dezsõ, B., Fekete, I., Pröhle, T.: \textit{A fully segment-based method for the classification of satellite images}, Annales Univ. Sci. Budapest, Sectio Computatorica 30, 2009.

\bibitem{bestMerge} Schoenmakers, R. P. H. M.: \textit{Integrated methodology for segmentation of large optical satellite images in land applications of remote sensing}, Office for Official Publications of the European Communities, 1995.

\bibitem{bestMerge2} Fekete I., Dezsõ B, László I., Ócsai K.: \textit{A szegmentálás szerepe az ûrfelvételek tematikus osztályozásában}, Informatika a felsõoktatásban konferencia, Debrecen, 2008.

\bibitem{toarefCompute} \textit{Converting Digital Numbers to Top of Atmosphere (ToA) Reflectance} http://www.yale.edu/ceo/Documentation/Landsat\_DN\_to\_Reflectance.pdf, 2016.

\bibitem{bhattacharyya} Bhattacharyya, A. \textit{On a measure of divergence between two multinomial populations} Sankhya: The Indian Journal of Statistics, 1946.

\bibitem{balancedHistogramThresholding} Anjos, A., Shahbazkia, H.: \textit{Bi-Level Image Thresholding - A Fast Method}, BIOSIGNALS, 2008.

\bibitem{bhtOptimal} Anjos, A., Shahbazkia, H., Leite R., Cancela, M. L.: \textit{MAQ - A Bioinformatics Tool for Automatic Macroarray Analysis}, International Journal of Computer Applications 7, 2010.

\bibitem{otsu}  Otsu, N.: \textit{A threshold selection method from gray-level histograms}, Automatica 11, 1975.

\bibitem{isodata} Ball, G. H., Hall, D. J.: \textit{ISODATA, a novel method of data analysis and pattern classification}, STANFORD RESEARCH INST MENLO PARK CA, 1965.
